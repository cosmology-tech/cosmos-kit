...
var DefaultModal = ({
  ...
  return /* @__PURE__ */ jsx9(ChakraProviderWithGivenTheme, { children: /* @__PURE__ */ jsx9(
    WalletModal,
    {
      isOpen,
      setOpen,
...
var WalletModal = ({
  ...
  const initialFocus = useRef2();
  const [currentView, setCurrentView] = useState2(
    ModalView.WalletList
  );
  const [qrState, setQRState] = useState2(State2.Init);
  const [qrMsg, setQRMsg] = useState2("");
  ...
  const onCloseModal = useCallback6(() => {
    setOpen(false);
    if (walletStatus === "Connecting") {
      current?.disconnect();
    }
  }, [setOpen, walletStatus, current]);
  const onReturn = useCallback6(() => {
    setCurrentView(ModalView.WalletList);
  }, [setCurrentView]);
  const modalView = useMemo5(() => {
    let ViewComponent;
    switch (currentView) {
...
var ChainProvider = ({
  ...
  const logger = useMemo6(() => new Logger(logLevel), []);
  ...
  const getChainProvider = (modal) => /* @__PURE__ */ jsx10(
    ChainProviderLite,
    {
      chains,
      assetLists,
  ...
  const defaultModal = useCallback7(
    (props) => /* @__PURE__ */ jsx10(
      WalletModal,
      {
        ...props,
        modalViews: {
  ...
  if (wrappedWithChakra) {
    ...
    return /* @__PURE__ */ jsx10(ChakraProviderWithOuterTheme, { logger, children: getChainProvider(defaultModal) });
  } else {
    logger.debug("Wrap with <ChakraProviderWithGivenTheme>.");
    return /* @__PURE__ */ jsx10(ChakraProviderWithGivenTheme, { theme: modalTheme, logger, children: getChainProvider(defaultModal) });
  }
};
