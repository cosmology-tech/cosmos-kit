...
var ChainWC = class extends import_core.ChainWalletBase {
  ...
  setClientNotExist() {
    this.setState(import_core.State.Error);
...
var WCClient = class {
  constructor(walletInfo) {
    ...
    this.qrUrl = { state: import_core2.State.Init };
    this.appUrl = { state: import_core2.State.Init };
  }
  get isMobile() {
  ...
  setQRError(e) {
    this.setQRState(import_core2.State.Error);
  ...
  async initSignClient() {
    ...
    this.signClient = await import_sign_client.default.init(this.options?.signClient);
  ...
  async initAppUrl() {
    this.appUrl.state = import_core2.State.Pending;
    if (!this.wcCloudInfo)
    ...
    this.appUrl.state = import_core2.State.Done;
  }
  get nativeUrl() {
  ...
  async connect(chainIds) {
    ...
    if (this.qrUrl.state !== "Init") {
      this.setQRState(import_core2.State.Init);
    }
    ...
    if (this.displayQRCode)
      this.setQRState(import_core2.State.Pending);
    ...
    try {
      ...
      if (this.displayQRCode)
        this.setQRState(import_core2.State.Done);
    } catch (error) {
    ...
    try {
      ...
      if (!error) {
        if (this.displayQRCode)
          this.setQRError(import_core2.ExpiredError);
        throw new Error("Proposal Expired");
      } else if (error.code == 5001) {
        throw import_core2.RejectedError;
      } else {
        throw error;
      }
    } finally {
      if (!pairing && this.qrUrl.message !== import_core2.ExpiredError.message) {
        this.setQRState(import_core2.State.Init);
      }
    }
  }
...
var WCWallet = class extends import_core4.MainWalletBase {
  constructor(walletInfo, ChainWC2, WCClient2) {
    ...
    this.clientMutable = { state: import_core3.State.Init };
    this.WCClient = WCClient2;
  }
