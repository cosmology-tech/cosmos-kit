...
var WCClient = class {
  constructor(walletInfo) {
    ...
    this.qrUrl = { state: import_core.State.Init };
    this.appUrl = { state: import_core.State.Init };
  }
  get isMobile() {
  ...
  setQRError(e) {
    this.setQRState(import_core.State.Error);
  ...
  async initAppUrl() {
    this.appUrl.state = import_core.State.Pending;
    if (!this.wcCloudInfo)
    ...
    this.appUrl.state = import_core.State.Done;
  }
  get nativeUrl() {
  ...
  async connect(chainIds) {
    ...
    if (this.qrUrl.state !== "Init") {
      this.setQRState(import_core.State.Init);
    }
    ...
    if (this.displayQRCode)
      this.setQRState(import_core.State.Pending);
    ...
    try {
      ...
      if (this.displayQRCode)
        this.setQRState(import_core.State.Done);
    } catch (error) {
    ...
    try {
      ...
      if (!error) {
        if (this.displayQRCode)
          this.setQRError(import_core.ExpiredError);
        throw new Error("Proposal Expired");
      } else if (error.code == 5001) {
        throw import_core.RejectedError;
      } else {
        throw error;
      }
    } finally {
      if (!pairing && this.qrUrl.message !== import_core.ExpiredError.message) {
        this.setQRState(import_core.State.Init);
      }
    }
  }
