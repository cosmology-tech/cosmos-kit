...
export class Repo<ItemRegistry extends Registry<Name>> {
    ...
    register(itemInfo: Name | ItemRegistry) {
        ...
        if (this.registeredItemMap.has(name)) {
            console.info(`Name ${name} already be registered, please choose another name.`);
            return;
        }
    ...
    get activeItems() {
        ...
        this.registeredItemMap.forEach(item => { item.active && items.push(item) });
        return items;
    }
    ...
    activateAll() {
        this.registeredItemMap.forEach(item => { item.active = true });
    }
    ...
    inactivateAll() {
        this.registeredItemMap.forEach(item => { item.active = false });
    }
}
