 ...
 */
export type SimplifiedKeplrChainInfo = Omit<
  ChainInfo,
...
export function createKeplrChainInfo({
  ...
  ...chainInfo
}: SimplifiedKeplrChainInfo): ChainInfo {
...
const KeplrSimpleChainInfoList: Record<ChainInfoID, SimplifiedKeplrChainInfo> =
  {
...
export const KeplrChainInfoMap = Object.entries(
  KeplrSimpleChainInfoList
).reduce(
  (curr, [id, simplifiedKeplrChainInfo]) => ({
    ...curr,
    [id]: createKeplrChainInfo(simplifiedKeplrChainInfo),
  }),
  {} as Record<`${ChainInfoID}`, ChainInfo>
)

export const getKeplrChainInfoLegacy = async (
  ...
  const chainInfo: ChainInfo | undefined =
    ...
    KeplrChainInfoMap[chainId]

  if (!chainInfo) {
    const availableChainIds = [
      ...(overrides?.map((info) => info.chainId) ?? []),
      ...Object.keys(KeplrChainInfoMap).filter(
