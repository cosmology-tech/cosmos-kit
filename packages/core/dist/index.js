...
__export(src_exports, {
  ChainWalletBase: () => ChainWalletBase,
  ClientNotExistError: () => ClientNotExistError,
  ExpiredError: () => ExpiredError,
  Logger: () => Logger,
  MainWalletBase: () => MainWalletBase,
  ModalView: () => ModalView,
  ...
  sum: () => sum,
  valuesApply: () => valuesApply
});
module.exports = __toCommonJS(src_exports);

// src/bases/chain-wallet.ts
...
var WalletBase = class extends StateBase {
  constructor(walletInfo) {
    super();
    this.clientMutable = { state: "Init" /* Init */ };
    /**
    ...
    this.disconnect = async (sync) => {
      await this._disconnect(sync);
    };
    this.connect = async (sync) => {
      ...
      try {
        if (!this.client) {
          ...
          await this.initClient(
            this.walletInfo.mode === "wallet-connect" ? this.walletConnectOptions : void 0
          );
...
var ChainWalletBase = class extends WalletBase {
  ...
  constructor(walletInfo, chainRecord) {
    ...
    this.rpcEndpoints = [];
    this.restEndpoints = [];
    this.namespace = "cosmos";
    this.getRpcEndpoint = async (isLazy) => {
      const lazy = getIsLazy(
        void 0,
        this.isLazy,
        this._rpcEndpoint?.isLazy,
        isLazy,
        this.logger
      );
      if (lazy) {
    ...
    this.getRestEndpoint = async (isLazy) => {
      const lazy = getIsLazy(
        void 0,
        this.isLazy,
        this._restEndpoint?.isLazy,
        isLazy,
        this.logger
      );
      if (lazy) {
    ...
    this.getStargateClient = async () => {
      const rpcEndpoint = await this.getRpcEndpoint();
      return import_stargate.StargateClient.connect(rpcEndpoint, this.stargateOptions);
    };
    this.getCosmWasmClient = async () => {
      const rpcEndpoint = await this.getRpcEndpoint();
      return import_cosmwasm_stargate.CosmWasmClient.connect(rpcEndpoint);
    };
    this.getNameService = async () => {
      const client = await this.getCosmWasmClient();
      const registry = getNameServiceRegistryFromChainName(this.chainName);
      return new NameService(client, registry);
    };
    this.getSigningStargateClient = async () => {
      const rpcEndpoint = await this.getRpcEndpoint();
      if (!this.offlineSigner) {
        await this.initOfflineSigner();
      }
      return import_stargate.SigningStargateClient.connectWithSigner(
        rpcEndpoint,
        this.offlineSigner,
        this.signingStargateOptions
      );
    };
    this.getSigningCosmWasmClient = async () => {
      const rpcEndpoint = await this.getRpcEndpoint();
      if (!this.offlineSigner) {
        await this.initOfflineSigner();
      }
    ...
    this.getSigningClient = async (type) => {
      switch (type) {
        case "stargate":
    ...
    this.estimateFee = async (messages, type, memo, multiplier) => {
      if (!this.address) {
        throw new Error(
      ...
      return (0, import_stargate.calculateFee)(
        Math.round(gasEstimation * (multiplier || 1.3)),
        gasPrice
      );
    };
    this.sign = async (messages, fee, memo, type) => {
      if (!this.address) {
        throw new Error(
          "Address is required to estimate fee. Try connect to fetch address."
        );
    ...
    this.broadcast = async (signedMessages, type) => {
      const client = await this.getSigningClient(type);
      const txBytes = import_tx.TxRaw.encode(signedMessages).finish();
    ...
    this.signAndBroadcast = async (messages, fee, memo, type) => {
  ...
  get chainLogoUrl() {
    return (
      ...
      this.chainRecord.assetList?.assets[0]?.logo_URIs?.svg || this.chainRecord.assetList?.assets[0]?.logo_URIs?.png || void 0
    );
  }
  get stargateOptions() {
...
var MainWalletBase = class extends WalletBase {
  constructor(walletInfo, ChainWallet) {
    ...
    this.getGlobalStatusAndMessage = (activeOnly = true) => {
      ...
      if (wallet)
        return ["Connecting" /* Connecting */, void 0];
      wallet = chainWalletList.find((w) => w.isWalletDisconnected);
      if (wallet) {
        return ["Disconnected" /* Disconnected */, "Exist disconnected wallets"];
      }
      wallet = chainWalletList.find((w) => w.isError || w.isWalletRejected);
      if (wallet)
        return [wallet.walletStatus, wallet.message];
      return ["Connected" /* Connected */, void 0];
    };
    ...
    this.emitter.on("reset", (chainIds) => {
      chainIds.forEach(
        (chainId) => Array.from(this.chainWalletMap.values()).find((cw) => cw.chainId === chainId)?.reset()
      );
    });
  }
  initingClient() {
...
var WalletRepo = class extends StateBase {
  constructor(chainRecord, wallets = []) {
    ...
    this.isActive = false;
    this.namespace = "cosmos";
    this.repelWallet = true;
    this.getWallet = (walletName) => {
    ...
    this.connect = async (walletName, sync) => {
      if (walletName) {
    ...
    this.disconnect = async (walletName, sync) => {
      if (walletName) {
...
var WalletManager = class extends StateBase {
  constructor(chains, assetLists, wallets, logger, throwErrors = false, defaultNameService, walletConnectOptions, signerOptions, endpointOptions, sessionOptions) {
    ...
    this.chainRecords = [];
    this.walletRepos = [];
    this.defaultNameService = "icns";
    this.mainWallets = [];
    this.repelWallet = true;
    this.addChains = (chains, assetLists, signerOptions, endpoints) => {
      const newChainRecords = chains.map(
        (chain) => convertChain(
          chain,
          assetLists,
          signerOptions,
    ...
    this.getChainWallet = (chainName, walletName) => {
      const chainWallet = this.getMainWallet(
        walletName
    ...
    this.getChainRecord = (chainName) => {
      const chainRecord = this.chainRecords.find(
        (c) => c.name === chainName
      );
      if (!chainRecord) {
        throw new Error(`${chainName} is not provided!`);
    ...
    this.getChainLogo = (chainName) => {
      ...
      return (
        ...
        chainRecord?.assetList?.assets[0]?.logo_URIs?.svg || chainRecord?.assetList?.assets[0]?.logo_URIs?.png || void 0
      );
    };
    this.getNameService = async (chainName) => {
      let _chainName;
      if (!chainName) {
      ...
      if (walletName) {
        ...
        if (accountsStr) {
          ...
          accounts.forEach((data) => {
            const chainWallet = mainWallet.getChainWalletList(false).find(
              (w) => w.chainRecord.chain.chain_id === data.chainId && w.namespace === data.namespace
            );
            chainWallet?.setData(data);
            chainWallet?.setState("Done" /* Done */);
          });
        }
      }
      if (walletName) {
