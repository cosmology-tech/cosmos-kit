...
| **getRpcEndpoint** | return rpc endpoint, if `isLazy` is `false` or `undefined`, will return *validated* endpoint    | **isLazy**?: `boolean` | `string` | Y |
| **getRestEndpoint**  | return rest endpoint, if `isLazy` is `false` or `undefined`, will return *validated* endpoint    | **isLazy**?: `boolean` | `string` | Y |
