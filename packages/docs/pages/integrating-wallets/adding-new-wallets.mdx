...
## About Wallet Logos
  We put all wallet logos under `cosmos-kit/public/images/logos/wallets` folder, and use them through `raw.githubusercontent.com` domain.

  When adding a new wallet, you need to specify the url of the wallet logo in `registry.ts`.  
  Add the logo file to `cosmos-kit/public/images/logos/wallets` first when necessary.  
  And set the value of `logo` to `https://raw.githubusercontent.com/cosmology-tech/cosmos-kit/main/public/images/logos/wallets/wallet.ext`
