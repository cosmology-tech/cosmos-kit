...
| Key      | Type | Description |
| ----------- | ----------- | -- |
| **rejectMessage**      | `string`   | `rejectMessage` is the error message string thrown by wallet app/extension when user rejects |
...
`MainWallet` is a class organizing all `ChainWallet`s. **It should extend  `MainWallet` class**, in which protected `_chainWallets` property stores all `ChainWallet`s.

#### Required methods

| Key      | Type | Description |
| ----------- | ----------- | -- |
| **getAccounts**      | `(chainId: string) => Promise<WalletAccount>`   | return account with address but without pubkey and algo |

#### Optional methods

| Key      | Type | Description |
| ----------- | ----------- | -- |
| **getAccounts**      | `(chainId: string) => Promise<WalletAccount>`<sup>*</sup>   | return account with address, pubkey and algo |
...
### 3️⃣ Extend `ChainWallet`

Create a `ChainWallet` class that extends  `ChainWallet`. `ChainWallet` provides wallet information for a particular chain, e.g. `address`, `offlineSigner`, etc. 

`ChainWallet` has implemented a bunch of methods such as wallet connection, sign, broadcast, etc. [[learn more]](#). Therefore, normally you don't need to do any extra work inside `ChainWallet`. But feel free to overwrite these methods or add new methods/properties if customization is needed to meet new demand.
...
### 4️⃣ Extend `MainWallet`

Create a `MainWallet` class that extends `MainWallet`. `MainWallet` organizes all `ChainWallet`s, which are stored in protected member `_chainWallets`.

> Note: Class `ChainWallet` created in [Step 3](#3️⃣-extend-chainWalletbase) is required in `MainWallet` construction.
...
Also, feel free to overwrite methods in `MainWallet` or add new methods/properties if necessary.
