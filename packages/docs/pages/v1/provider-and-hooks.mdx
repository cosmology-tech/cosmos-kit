...
- [`ChainProvider`](/provider-and-hooks/chain-provider)

## Hooks

There are multiple hooks provided in CosmosKit. They all require [**ChainProvider**](/provider-and-hooks/chain-provider) from either `@cosmos-kit/react` or `@cosmos-kit/react-lite` to provide necessary information.
