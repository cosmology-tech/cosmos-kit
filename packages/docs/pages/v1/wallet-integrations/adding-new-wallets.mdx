...
| Key      | Type |
| ----------- | ----------- |
| **initClient**      | `(options?: any) => void \| Promise<void>`<sup>*</sup>   |
...
Last but not least, append this instance to the `wallets` property of [ChainProvider](/provider-and-hooks/chain-provider).