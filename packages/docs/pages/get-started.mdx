...
### Try Use Beta Version of CosmosKit

In our beta version of CosmosKit, we discard the dependency on `ChakraUI` due to numerous runtime errors and the cost of CSS-in-JS.

Main changes lies on `@cosmos-kit/react`, where the default modal locates. All the modal components are imported from `@cosmology-ui/react`, which is our UI Kit in Cosmology. The latest `@cosmology-ui/react` discards `ChakraUI` and instead adopts a pure CSS styling solution through a package called `vanilla-extract`.
