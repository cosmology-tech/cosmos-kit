...
In our prerelease version of CosmosKit, we used `tsup` to compile CommonJS, ESM and types. Different from normal versions that separate these files in different folders (`main`, `module` and `types`), in the prerelease version, they are put in a single folder `dist` and differentiate with different suffix (`.js`, `mjs` and `.d.ts`). In this way non-index sources can easily find its corresponding typings.
