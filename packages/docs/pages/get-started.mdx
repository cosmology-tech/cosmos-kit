...
> Note: `@cosmjs/*` packages are peer dependencies in `@cosmos-kit/core`, so if you're not using `@cosmjs/*` in your dapp, yarn would complain UNMET cosmjs peer dependencies.
