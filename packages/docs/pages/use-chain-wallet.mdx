...
| **getRpcEndpoint** | return rpc endpoint, if `isLazy` is `true`, will return *validated* endpoint    | **isLazy**?: `boolean` | `string` | Y |
| **getRestEndpoint**  | return rest endpoint, if `isLazy` is `true`, will return *validated* endpoint    | **isLazy**?: `boolean` | `string` | Y |
