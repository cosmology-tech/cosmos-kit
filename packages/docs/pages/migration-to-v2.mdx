...
Customizing the default modal styles is done through `customDefaultModalProps` prop of `<ChainProvider />`

We are currently providing 2 ways to customize the default modal:

1. Overriding the theme object:

By using the `customDefaultModalProps.themeDefs` and `customDefaultModalProps.customTheme`, you can override all of the theme tokens however you want
...
This is done through the `customDefaultModalProps.overrides` prop, which is a record with keys corresponding to a slot names and values are objects of overridable property name and its value in each theme mode.
