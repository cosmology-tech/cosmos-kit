...
`@cosmos-kit/react` includes default modal made by `ChakraUI`. If [customized modal](/provider-and-hooks/chain-provider/#customize-modal-with-walletmodal) is providered, you can use `@cosmos-kit/react-lite` instead to lighter your app. 

### 2️⃣ Wrap Provider

First, add [`ChainProvider`](/provider-and-hooks/chain-provider) and provider [required properties](/provider-and-hooks/chain-provider#required-properties).
