...
- [`useChain`](https://docs.cosmoskit.com/use-chain): Provides chain wallet related properties and methods, and supports multiple chains connected at one time. When `useChain` is called, corresponding chain wallet will be activated.

- [`useManager`](https://docs.cosmoskit.com/use-manager): Manager for all chains and wallets.

> Note: 
> 1. If you are using `Chakra UI` in your app, make sure `ChainProvider` is wrapped inside `ChakraProvider` and [`wrappedWithChakra`](#wrappedwithchakra) property in `ChainProvider` is set `true`.
