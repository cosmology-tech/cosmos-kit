...
- [`useChain`](/use-chain): Provide chainWallet related properties and methods, and supports multiple chains connected at one time. When `useChain` is called, corresponding chain wallet will be activated.
...
These properties are shared by `ChainProvider` from `@cosmos-kit/react` and `@cosmos-kit/react-lite`.
...
> This property is **required** if `ChainProvider` is imported from `@comos-kit/react-lite`, and is **optional** if imported from `@comos-kit/react`.
