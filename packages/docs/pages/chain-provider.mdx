...
`isLazy` is used to control endpoint validation.

- When `isLazy` is `false`, will do endpoint validation
- When `isLazy` is `true`, will **disable** endpoint validation
- When `isLazy` is `undefined`, will inherit higher level explicitly set `isLazy`. If none is explicitly set, will do endpoint validation
...
The calculation of final `isLazy` can be seen [here](https://github.com/cosmology-tech/cosmos-kit/blob/main/packages/core/src/utils/endpoint.ts).
...
These properties **only** exist in `ChainProvider` from `@cosmos-kit/react`, and only works when property [`walletModal`](#walletmodal) is `undefined`.
...
Optional property. By default `false`, which means on mobile only wallets with registry property `mobileDisabled` is set `false` or `undefined` be displayed on wallet list page of default modal. For example, all `extension` wallets are set `mobileDisabled` `true`. Therefore you can't see `extension` wallets on mobile by default. If you want to see all wallets on mobile, set `includeAllWalletsOnMobile` `true`;
