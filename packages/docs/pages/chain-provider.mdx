...
Optional property of type `({ isOpen, setOpen, walletRepo, theme }: WalletModalPropsV2) => JSX.Element`
...
Optional property. By default `false`. If set `true`, will print `LOG`, `INFO` or `WARNING` on console.
