...
export class VectisClient implements WalletClient {
  ...
  async addChain({ chain, name, preferredEndpoints }: ChainRecord) {
    ...
    const chainInfo: VectisChainInfo = {
      ...
      rpcUrl: preferredEndpoints.rpc[0] || chain.apis.rpc[0].address,
      restUrl: preferredEndpoints.rest[0] || chain.apis.rest[0].address,
      bech32Prefix: chain.bech32_prefix,
      bip44: {
        coinType: chain.slip44,
      },
      defaultFeeToken: firstFeeToken.denom,
      defaultGasPrice: firstFeeToken.average_gas_price,
      testnet: chain.network_type === 'testnet',
      stakingToken: chain.staking.staking_tokens[0].denom,
      feeTokens: chain.fees.fee_tokens.map((feeToken) => ({
        denom: feeToken.denom,
        coinDecimals: 6,
      })),
      gasPriceStep: {
        low: firstFeeToken.low_gas_price,
        average: firstFeeToken.average_gas_price,
        high: firstFeeToken.low_gas_price,
      },
    };
